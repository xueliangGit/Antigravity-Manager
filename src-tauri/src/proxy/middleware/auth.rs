...
async fn auth_middleware_internal(
    ...
    // 静态文件豁免（前端资源）
    let is_static_file = path.starts_with("/assets/")
        || path.starts_with("/font/")
        || path.starts_with("/images/")
        || path == "/index.html"
        || path == "/";

    if !path.contains("event_logging") && !is_health_check && !is_static_file {
        tracing::info!("Request: {} {}", method, path);
    } else {
        tracing::trace!("Static/Heartbeat/Health: {} {}", method, path);
    }
    ...
    // 静态文件直接放行，无需认证
    if is_static_file {
        tracing::debug!("Static file bypassed auth: {}", path);
        return Ok(next.run(request).await);
    }
